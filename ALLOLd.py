...
# Time Succes decompile : 2022-06-19 20:51:15.873631
W = '\033[97;1m' 
...
def helpnote():
	...
	print("%s [*] JOIN MY wa GROUP  To KNOW ABOUT UPDATES  :)"%(G))

	subprocess.check_output(["am", "start", "https://wa.me/+923494462675"])

	exit(" [*] Whatsapp :  https://wa.me/+923494462675")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923494462675 ")
...
class Main:
	...
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://pastebin.com/j8WnH7LT').text
...
I CAN'T LIVE WITHOUT THEM🤷‍♂️
    ┌───────────────────────────────────────┐
    │ [✓] AUTHOR   : HAROON AHMAD                                                    │
    │ [✓] GITHUB   : Haroon302                                                                 │
    │ [✓] WHATSAPP :  +923494462675                                                  │
    │ [✓] POWERED BY : \x1b[1;32mHAROON PATHAAN\x1b[1;97m │
    └───────────────────────────────────────┘  """)

		print("%s [%s®%s] %sTOOL NAME : %sACCOUNT CLONER"%(G,R,G,Y,G))

		print("%s [%s®%s] %sVERSION   : %s2.1 "%(G,R,G,Y,G))
		...
		print("\033[92;1m         HAROON AHMAD     ")

		print("\033[97;1m   ───────────────────────────────────────")

		print("\n%s [%s1%s]%s CRACK RANDOM FB ID 2008-11 %s(FREE)"%(G,R,G,Y,W))

		print("%s [%s2%s]%s CRACK FB ID 2015-22 %s(PAID 100 PKR) V1"%(G,R,G,Y,G))

		print("%s [%s3%s]%s CRACK FROM NUMBER  %s(PAID 100 PKR) VIP"%(G,R,G,Y,G))

		print("%s [%s4%s]%s CRACK FB ID 2004-5%s(150 PKR) VIP-1"%(G,R,G,Y,G))

		print("%s [%s5%s]%s CRACK FROM EMAILS %s(150 PKR)"%(G,R,G,Y,G))

		print("%s [%s6%s]%s CRACK FB ID Custom %s(200 PKR) V1"%(G,R,G,Y,G))

		print(GET)

		print("\033[97;1m   ───────────────────────────────────────")

		print("\033[92;5m         GIVE RESPECT     TAKE RESPECT    ")
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [+] OK RESULTS SAVED IN -> HAROON-ok.txt"%(G))

				print("%s [+] CP RESULTS SAVED IN -> HAROON-cp.txt"%(Y))
	...
	def old4_7(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [!] USE %s, %s(COMMA)%s FOR SEPARATE "%(Y,G,B,Y))
	...
	def email(self):
		...
		nam = input("%s [?] TYPE A NAME %s(EX:haroon ", " pathaan): "%(Y,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			...
			"\r\r %s\033[0;93m[HAROON ] : \033[0;97m %s/%s -> \033[0;92m [OK:%s ]- \033[0;93m[CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;93m[HAROON-CP] %s|%s\033[0;97m         "%(uid, pw))
